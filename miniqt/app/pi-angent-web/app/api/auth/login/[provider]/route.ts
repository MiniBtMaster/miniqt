...
export async function POST(
  ...
  if (!token || !code) {
    return Response.json({ error: "缺少 token 或 code" }, { status: 400 });
  }

  const registry = getCallbackRegistry();
  const callbacks = registry.get(token);
  if (!callbacks) {
    return Response.json({ error: "没有与此 token 对应的登录流程" }, { status: 404 });
  }
    ...
    async start(controller) {
      ...
      const cleanup = () => {
        for (const token of activeTokens) {
          registry.get(token)?.reject(new Error("登录已取消"));
          registry.delete(token);
        }
        activeTokens.clear();
      };
