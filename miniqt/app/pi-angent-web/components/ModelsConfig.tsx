    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <Field label="API Key">
        <SecretTextInput value={provider.apiKey ?? ""} onChange={(v) => set("apiKey", v || undefined)}
          placeholder="环境变量名、!shell-command 或明文 key" mono />
        <span style={{ fontSize: 10, color: "var(--text-dim)", marginTop: 2 }}>
          以 <code style={{ fontFamily: "var(--font-mono)" }}>!</code> 开头可运行 shell 命令，也可以填写环境变量名
        </span>
      </Field>
      ...
      <div style={{ minHeight: 48 }}>
        {loginState.phase === "idle" && (
          <p style={{ margin: 0, fontSize: 12, color: "var(--text-muted)", lineHeight: 1.5 }}>
            {provider.loggedIn ? "已连接。你可以重新登录或断开连接。" : `连接你的 ${provider.name} 账号。`}
          </p>
        )}
...
function ApiKeyDetail({ provider, onRefresh }: { provider: ApiKeyProvider; onRefresh: () => void }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <p style={{ margin: 0, fontSize: 12, color: "var(--text-muted)", lineHeight: 1.5 }}>
        {provider.configured
          ? `API Key 已保存。可以在下方输入新 Key 进行替换，或断开连接来移除。`
      ...
      <Field label="API Key">
        <div style={{ display: "flex", gap: 6 }}>
          <SecretTextInput
            ...
            placeholder={provider.configured ? "输入新 Key 进行替换…" : "sk-…"}
            style={{ flex: 1 }}
            autoComplete="off"
            spellCheck={false}
            mono
              ...
              {availableOAuth.map((p) => (
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 10, color: "var(--text-dim)", marginTop: 2 }}>OAuth 登录</div>
                  </div>
