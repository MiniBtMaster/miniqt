...
export function SessionSidebar({ selectedSessionId, onSelectSession, onNewSession, initialSessionId, onInitialRestoreDone, refreshKey, onSessionDeleted, onSessionMerged, selectedCwd: selectedCwdProp, onCwdChange, onOpenFile, explorerRefreshKey, onAtMention, autoSession }: Props) {
  ...
  useEffect(() => {
    ...
    if (selectedCwd === null) {
      ...
      if (cwds.length > 0) {
        ...
        // 嵌入式模式：自动打开最近的会话
        if (autoSession && !initialSessionId && !restoredRef.current) {
          restoredRef.current = true;
          const recentSessions = [...allSessions]
            .filter((s) => s.cwd === cwds[0])
            .sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
          if (recentSessions.length > 0) {
            onSelectSession(recentSessions[0], true);
            return;
          }
        }
      }
    }
  }, [allSessions, selectedCwd, initialSessionId, onSelectSession, onInitialRestoreDone, autoSession]);
          ...
          <div style={{ display: "flex", alignItems: "center", flexShrink: 0 }}>
            ...
            <button
              onClick={() => {
                ...
                // 通知 miniqt 刷新 code_editor_window 的文件树
                console.log('__PI_REFRESH_FILES__');
              }}
              title="刷新文件列表"
              style={{
                display: "flex", alignItems: "center", justifyContent: "center",
