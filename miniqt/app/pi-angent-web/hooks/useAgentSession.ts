...
export interface UseAgentSessionOptions {
  ...
  /** 当模型通过 bash 执行 python *.py 时触发，将文件路径通知 PyQt 以便在 miniqt 中打开并运行回测 */
  onBashPythonFile?: (filePath: string) => void;
...
export function useAgentSession(opts: UseAgentSessionOptions) {
  const {
    session, newSessionCwd, onAgentEnd, onSessionCreated, onSessionForked, onFileChanged,
    onBashPythonFile, modelsRefreshKey, onBranchDataChange, onSystemPromptChange,
  } = opts;
  ...
  const notifiedFilesInTurnRef = useRef<Set<string>>(new Set());
  const toolArgsRef = useRef<Map<string, Record<string, unknown>>>(new Map());  // toolCallId → args
  const onFileChangedRef = useRef(onFileChanged);
  onFileChangedRef.current = onFileChanged;
  const onBashPythonFileRef = useRef(onBashPythonFile);
  onBashPythonFileRef.current = onBashPythonFile;
  const sessionCwdRef = useRef(session?.cwd ?? null);
  sessionCwdRef.current = session?.cwd ?? null;
  ...
  const handleAgentEvent = useCallback((event: AgentEvent) => {
    switch (event.type) {
      case "agent_start":
        ...
        dispatch({ type: "start" });
        notifiedFilesInTurnRef.current = new Set();
      ...
      case "message_end": {
        ...
        // 兜底：扫描 assistant 消息中的 write/edit toolCall，通知 PyQt 刷新代码编辑器
        // 用于处理 tool_execution_start 中未携带 args 的工具（如 edit）
        if (completed && completed.role === "assistant" && onFileChangedRef.current) {
          const blocks = completed.content as Array<{
            type: string;
            toolCallId?: string;
            toolName?: string;
          ...
          for (const block of blocks) {
            if (block.type === "toolCall" && (block.toolName === "write" || block.toolName === "edit")) {
              ...
              if (filePath && typeof filePath === "string" && filePath.endsWith(".py")) {
                if (!notifiedFilesInTurnRef.current.has(filePath)) {
                  notifiedFilesInTurnRef.current.add(filePath);
                  onFileChangedRef.current([filePath]);
                }
              }
            }
          }
      ...
      case "tool_execution_start": {
        ...
        // 缓存 args，供 tool_execution_end 提取文件路径
        if ((name === "write" || name === "edit") && event.args) {
          toolArgsRef.current.set(id, event.args as Record<string, unknown>);
        }
        // 检测 bash python *.py 命令，通知 PyQt 打开并运行回测
        if (name === "bash" && event.args?.["command"] && typeof event.args["command"] === "string") {
          const cmd: string = event.args["command"];
          const pyMatch = cmd.match(/(?:python(?:3)?|py)\s+(.+?\.py)(?:\s|;|$)/);
          if (pyMatch && onBashPythonFileRef.current) {
            let pyFile = pyMatch[1].trim();
            // 解析相对路径：优先使用命令中的 cd 目录，其次用会话 cwd
            const cdMatch = cmd.match(/cd\s+(?:"([^"]+)"|(\S+))/);
            const cdDir = cdMatch ? (cdMatch[1] ?? cdMatch[2]) : null;
            const cwd = cdDir ?? sessionCwdRef.current;
            if (cwd && !pyFile.match(/^[A-Za-z]:[/\\]/) && !pyFile.startsWith("/")) {
              pyFile = `${cwd.replace(/[\\/]+$/, "")}/${pyFile}`;
            }
            onBashPythonFileRef.current(pyFile);
          }
        }
        setAgentPhase((prev) => {
      ...
      case "tool_execution_end": {
        const id = event.toolCallId as string;
        const name = event.toolName as string;
        // 工具执行完成后，通知 PyQt 刷新代码编辑器
        if ((name === "write" || name === "edit") && onFileChangedRef.current) {
          const args = toolArgsRef.current.get(id);
          if (args) {
            const filePath = args.path ?? args.file_path;
            if (filePath && typeof filePath === "string" && filePath.endsWith(".py")) {
              const p = filePath;
              if (!notifiedFilesInTurnRef.current.has(p)) {
                notifiedFilesInTurnRef.current.add(p);
                onFileChangedRef.current([p]);
              }
            }
          }
          toolArgsRef.current.delete(id);
        }
        setAgentPhase((prev) => {
