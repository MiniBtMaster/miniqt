...
from .util import *
from qfluentwidgets import RoundMenu,Action,LineEdit,FluentIcon

class Terminal(QTextEdit, VT100Paser):
    """ 基于QTextEdit实现的终端小部件 """
    text_added = pyqtSignal(str)
    cd_to_file_tree_path = pyqtSignal()  # 右键菜单：切换至文件树当前路径

    def __init__(self,parent=None, bg_img=None, scheme=None):
        super().__init__(parent)

        self.bg_img = bg_img
        scheme = scheme if scheme else "Ocean Dark"
        ...
        # 快捷键
        self.shortcut_copy = QKeySequence("Ctrl+C")
        self.shortcut_paste = QKeySequence("Ctrl+V")
    ...
    def getExtraSelectedText(self):
        ...
        for selection in self.extraSelections():
            selected_text += selection.cursor.selectedText()
    ...
    def keyPressEvent(self, event: QKeyEvent):
        ...
        if event.key() == Qt.Key.Key_Up:        # 上键
            text = '\x1b[A'
        elif event.key() == Qt.Key.Key_Down:    # 下键
            text = '\x1b[B'
        elif event.key() == Qt.Key.Key_Right:   # 右键
            text = '\x1b[C'
        elif event.key() == Qt.Key.Key_Left:    # 左键
            text = '\x1b[D'
        elif event.key() == Qt.Key.Key_C and event.modifiers() & Qt.KeyboardModifier.ControlModifier:
            self.copy()
            return
        elif event.key() == Qt.Key.Key_Delete:  # Del键
            text = '\x1b[3~'
    ...
    def mousePressEvent(self, event: QMouseEvent):
        """ 鼠标点击事件 """
        if event.button() == Qt.MouseButton.MiddleButton and event.modifiers() & Qt.KeyboardModifier.ControlModifier:
            font = self.font()
            font.setPointSize(11)
            self.setFont(font)
            return
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        """ 鼠标移动事件 """
        if event.buttons() & Qt.MouseButton.LeftButton:  # 按住鼠标左键移动
            self.setExtraSelections([])
            format = QTextCharFormat()
            format.setBackground(self.selected_color)
            ...
            selection.cursor.setPosition(self.left_click_pos, QTextCursor.MoveMode.KeepAnchor)
            selections = self.extraSelections()
            selections.append(selection)
            self.setExtraSelections(selections)
    ...
    def find(self):
        if not self.find_bar:
            self.find_bar = LineEdit(self)
            palette = QPalette()
            palette.setColor(QPalette.ColorRole.Base, QColor(self.scheme['Foreground']))
            palette.setColor(QPalette.ColorRole.Text, QColor(self.scheme['Background']))
    ...
    def input(self):
        ...
        self.input_bar.setGeometry(int((self.width()-width)/2), int(self.height()-height-20), int(width), int(height))
    ...
    def contextMenuEvent(self, event):
        ...
        copy_action.setIcon(FluentIcon.COPY.icon())#QIcon(QIcon.fromTheme(QIcon.ThemeIcon.EditCopy)))
        if not extra_selected:
            copy_action.setDisabled(True)
        
        paste_action = Action("粘贴", menu)
        paste_action.setShortcut(self.shortcut_paste)
        paste_action.setIcon(FluentIcon.PASTE.icon())#QIcon(QIcon.fromTheme(QIcon.ThemeIcon.EditPaste)))
        if not clipboardhas_content:
            paste_action.setDisabled(True)

        clear_action = Action("清除", menu)
        clear_action.setShortcut(self.shortcut_clear)
        clear_action.setIcon(FluentIcon.CUT.icon())#QIcon(QIcon.fromTheme(QIcon.ThemeIcon.EditClear)))

        find_action = Action("查找", menu)
        find_action.setShortcut(self.shortcut_find)
        find_action.setIcon(FluentIcon.SEARCH.icon())#QIcon(QIcon.fromTheme(QIcon.ThemeIcon.EditFind)))

        input_action = Action("输入栏", menu)
        input_action.setShortcut(self.shortcut_input)
        input_action.setIcon(FluentIcon.SEND.icon())#QIcon(QIcon.fromTheme(QIcon.ThemeIcon.DocumentSend)))
        ...
        menu.addSeparator()
        # 切换至文件树路径
        cd_action = Action("切换至文件树路径", menu)
        cd_action.setIcon(FluentIcon.FOLDER.icon())
        cd_action.triggered.connect(self.cd_to_file_tree_path.emit)
    ...
    def updateText(self, text: str):
        ...
        cursor.setPosition(cursor.position()+cnt, QTextCursor.MoveMode.KeepAnchor)
        cursor.removeSelectedText()
